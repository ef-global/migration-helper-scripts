...
REPO="${MIGRATION_HELPER_REPO:-ef-global/migration-helper-scripts}"
VERSION="${MIGRATION_HELPER_VERSION:-latest}"
INSTALL_DIR="${MIGRATION_HELPER_INSTALL_DIR:-/usr/local/bin}"

usage() {
  cat <<EOF
Install migration-helper from GitHub Releases.
...
Environment variables:
  MIGRATION_HELPER_VERSION
  MIGRATION_HELPER_INSTALL_DIR
  MIGRATION_HELPER_REPO
EOF
}
...
echo "Installing migration-helper from ${REPO} ${TAG}..."
...
if [ ! -f "${BIN_PATH}" ]; then
  echo "Downloaded asset did not contain expected binary: ${BIN_PATH}" >&2
  exit 1
fi
...
mkdir -p "${INSTALL_DIR}"
install -m 0755 "${BIN_PATH}" "${INSTALL_DIR}/migration-helper"

echo "Installed to ${INSTALL_DIR}/migration-helper"
echo "Run: migration-helper help"
